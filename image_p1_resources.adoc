...
not yet possible to properly attribute Creative Commons photos used
in the <<chapter_image_intro, original presentation of iMage>>,
therefore for now I'll use a set of free photos of castles from
...
to XHTML. By default it recognizes a subset of XHTML and renders
...
extensible. That means one can easily add arbitrarily complex functionality
by adding interpretations for some additional tags.
...
http://stackoverflow.com/questions/940382/haskell-difference-between-dot-and-dollar-sign[+$+ operator]). +|>+ operator make it possible to nicely chain function calls.
...
https://github.com/akoprow/iMage/blob/83d5723c3b6993a82e0d7b4219642ca3b6c3bf6a/footer.xmlt[this content]
in the current directory. And then put together what we discussed
so far and create +iMage.opa+ with the following content:
...
is important is to realize that the produced executable (+iMage.exe+)
...
be an XHTML page, but also an image or a style-sheet file.
...
to those embedded files. The http://en.wikipedia.org/wiki/Mime-type[MIME type] of the
embedded resource is determined by the compiler based on the file extension.
...
How to provide CSS for services developed with Opa? In a sense
we already saw one way to do that.

*External CSS*: put an external CSS file in some
directory, include it with +@static_resource_directory+ and provide
an URL in +Server.one_page_bundle+. So if we assume there is a
+style.css+ file in +resources/css+ then the above program could
be extended with this style by replacing its server declaration with:
...
There's one useful trick. If you compile your program in the development
mode (i.e. without the +--compile-release+ switch) then adding +-d+ (or
+--debug-editable-all+) switch when running the server will have the
following effect.

* Directory +opa-debug+ will be created containing all the files embedded
  in the server.
* Changing those files will have *immediate* effect on the running server,
  so for instance one can tweak the CSS files and see the changes
  immediately (well, after reloading pages in the browser that is).
  This can be quite useful during the development process.
* Those files are temporary so if you want your modified files to be
  embedded in the server, you still need to update the included
  resources and recompile the server.

There are more options for fine-tuning which resources are exposed
via this process; try running your server with +--help+ switch and
check the ``Debugging Resources : dynamic edition'' section.

*Inline CSS.*
Second way of working with CSS is directly in the Opa source code.
CSS is a data-type in Opa (+Css.properties+ for a set of CSS properties
and +Css.declaration+ for a list of CSS declarations) just like XHTML
is (+xhtml+). There is also a special syntax to declare CSS properties
in Opa +css { ... }+ where the three dots can essentially be replaced
with usual CSS syntax but additionally it is possible to use Opa
inserts inside this CSS. So for instance to declare a block with
the width of 100 pixels per every image we have we could write:
...
[NOTE]
=======================
The inline CSS is currently limited in Opa and only accepts some
subset (rather large) of CSS3 (for instance +:first-child+ special
selector is not yet supported, nor is the +text-shadow+ property).
On the other hand Opa tries to provide cross-browser compatiblity
for inlined CSS so for instance the single +opacity+ property will
be translated into a
http://www.w3schools.com/css/css_image_transparency.asp[number of declarations]
to ensure proper interpretation by all major browsers. The external
CSS, on the other hand, is included as-is.
=======================

Moreover just like +server+ declaration has the special role of
declaring a server; similarly +css+ declaration is used to declare
(service-wide) CSS. So we could for instance write:
...
[NOTE]
=======================
There are currently few syntactical limitations to inlined CSS.
The +css+ block should be placed at the end of the Opa file and
it's safer to terminate all properties with an ending semicolon (+;+).
Yes, Opa is a young, rapidly changing language.
No, it's not perfect yet :).
=======================

It's of course also perfectly fine to mix the two styles, which we
will also do: external CSS for static properties and inline CSS
for dynamically computed properties.

Putting it all together
~~~~~~~~~~~~~~~~~~~~~~~

Huh, I realized only now that this became a veery long (too long?) post.
But the good news is that if we put together all that we learned so far
we are going to obtain a very basic version of
http://iMage.opalang.org/?images=http://image.opalang.org/resources/img-data-thefuturebuzz.txt[iMage]...
don't worry fancy user interface and other goodies will come in future
posts :).
...
The remaining changes is just providing some mark-up that allows
us to write
https://github.com/akoprow/iMage/blob/ebd7bf1b78f20f94317fdc914b05d66fad26dc2e/resources/css/css.css[this CSS]
to make things look acceptable. Btw. to avoid
misunderstanding: I have a basic working knowledge of CSS, but I'm by
no means no expert, neither in writing CSS, nor in designing nice looking
pages. So if something looks crappy -- blame me not Opa ;). Also I'm not going
to explain the CSS that I'm using (this blog is about *developing*
in Opa and if you ask me, CSS is better left for designers).

Ok, below is the complete code of the application. In the next post we're
either:

* going to develop a simple configuration format and write a parser for it,
  in order to be able to easily configure a set of images to display,
...
P.S. I think the rendering in Google Reader should be slightly better now.
Feedback welcome.
