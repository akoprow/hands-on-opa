...
I know, long time no post... I just came back from holidays and will try to get some fresh, new content ready for the blog.

In the <<chapter_sessions, last post>> I talked about the importance of sessions. To illustrate that in this post I'll show you first how to modify the http://tutorials.opalang.org/iMage[iMage application] we <<chapter_image_p3_parsing, had so far>> to use sessions as the way of invoking and handling events. Then we'll extend this application with a new feature: it will automatically accomodate to the size of the browser window. I'll also explain why it would be difficult to add this feature in the previous non-sessions-based setting. Ready?
...
* More about sessions by using them for the http://tutorials.opalang.org/iMage[iMage application].
...
There are only few small changes we need to introduce. If you forgot what we learned about sessions (or did not read the previous post) I suggest you skim over the <<chapter_sessions, post where I introduced them>>.
...
So essentially on clicking the thumbnail it directly calls +show_img+ to display the full version of the clicked thmbnail. However now instead of performing the action we will just generate an event. So that function needs to take an extra parameter, which is the slideshow session and it becomes:
...
<span class="run"><A target="_blank" href="http://tutorials.opalang.org/iMage-06">Run</A></span>
++++
