...
page() = <span id=#time onready={do_onready} />
------------------------
...
[source, opa]
------------------------
page() = <span id=#time onready={_ -> Scheduler.timer(1000, show_time)} />
------------------------
...
program would stop working as expected. Therefore it is a good practice
to declare all DOM ids used on a given page as constant and then only
refer to them (which means the above scenarion cannot happen anymore).
